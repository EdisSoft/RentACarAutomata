...
namespace FunctionsCore.Utilities.Extension.StringExtension
{
    public static class StringExtensions
    {
        public static string ReplaceAndClearHtmlTag(this string input) 
        {
            input = input.Replace("</p>", Environment.NewLine);
            input = input.Replace("<p>", string.Empty);
            input = input.Replace("&nbsp;", " ");
            input = input.Replace("<br>", Environment.NewLine);
            input = input.Replace("<br/>", Environment.NewLine);
            input = input.Replace("<br />", Environment.NewLine);
            input = input.ClearHTMLTags();
            return input;
        }

        public static string ClearHTMLTags(this string input)
        {
        ...
        {
            bool regexValid = Regex.Match(number, @"^\+[1-9]{1}[0-9]{3,14}$").Success;
            return number.Length >= 6 && regexValid;
        }
        ...
        public static List<string> JsonToList(this string jsonList)
        {
            if (string.IsNullOrEmpty(jsonList))
                return new List<string>();
            List<string> list = JsonConvert.DeserializeObject<List<string>>(jsonList);
            return list;
        }

        public static List<string> SpaceTordeles(this string szoveg)
        {
            if (szoveg == null)
                return new List<string>();
            return szoveg.Split(" ").OrderByDescending(o => o.Length).Select(s => s.ToLower()).ToList();
        }

        public static int GetOriginalLengthInBytesFromBase64String(this string base64string)
        {
        ...
        public static string ConcatStringNormalize(this string s)
        {
            return s.TrimEnd(' ').TrimEnd(',').Replace(", , ,", ",").Replace(", ,", ",");
        }

        public static string EmptyStringToKotojel(this string s)
        {
        ...
        public static string IfEmptyChange(this string original, string newStr)
        {
        ...
        public static string TelefonszamJsonToTelefonszamString(this string teloszam)
        {
            var telefonszamokList = teloszam.JsonToList();
            var telefonszamok = String.Join(", ", telefonszamokList.ToArray());
        ...
        public static bool ContainsAnyInvalidCharacters(this string fileName)
        {
            var result = !string.IsNullOrEmpty(fileName) && fileName.Any(Path.GetInvalidFileNameChars().Contains);
            return result;
        }
        ...
        public static string SetKorabbiAzonositok(string azonositok)
        {
            //a korábbi ideiglenes azonosítók nem kellenek a gyorskeresőbe
            if (string.IsNullOrWhiteSpace(azonositok))
                return string.Empty;

            string[] azonArray = azonositok.Split(",");

            var asd = azonArray.Any(a => a.Contains("-"));
            var splittedAzon = azonArray.Where(a => a.Contains("-"));
            string result = string.Join(",", splittedAzon);
        ...
        public static string EscapeUriString(this string uri)
        {
            ...
            var splitted = uri.Split(new char[] { '/', '\\' }).ToList();
            var lastEscaped = Uri.EscapeDataString(splitted.Last());

            if (splitted.Count == 1) return lastEscaped;

            splitted.RemoveAt(splitted.Count - 1);
            splitted.Add(lastEscaped);

            return string.Join('/', splitted);

        }

        public static string UnescapeUriString(this string uri)
        {
            if (string.IsNullOrWhiteSpace(uri)) return uri;

            var splitted = uri.Split(new char[] { '/', '\\' }).ToList();
            var lastEscaped = Uri.UnescapeDataString(splitted.Last());
        ...
        public static int GetInt32HashCode(this string strText)
        {
            if (string.IsNullOrEmpty(strText)) return 0;
            byte[] byteContents = Encoding.Unicode.GetBytes(strText);
            byte[] hashText = new System.Security.Cryptography.SHA1CryptoServiceProvider().ComputeHash(byteContents);
        ...
        public static string EscapeBase64FromJson(this string json)
        {
            var base64Regex = new Regex("\"[a-zA-Z0-9/+=]{500,}\"", RegexOptions.Multiline);
            return base64Regex.Replace(json, "\"BASE64\"");
        }
    }
