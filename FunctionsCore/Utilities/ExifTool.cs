...
namespace FunctionsCore.Utilities
{
    public static class ExifTool
    {
        ...
        public static Image OrientImage(Image img)
        {
            ...
            return SetImageOrientation(img, ExifOrientations.TopLeft);
        }

        // Set the image's orientation.
        public static Image SetImageOrientation(Image img, ExifOrientations orientation)
        {
            // Get the index of the orientation property.
            int orientation_index = Array.IndexOf(img.PropertyIdList, OrientationId);

            // If there is no such property, do nothing.
            if (orientation_index < 0) return img;
